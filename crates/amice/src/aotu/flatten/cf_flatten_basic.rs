...
use amice_plugin::inkwell::llvm_sys::prelude::LLVMBasicBlockRef;
...
fn do_handle(module: &mut Module<'_>, function: FunctionValue, demote_switch: bool) -> anyhow::Result<()> {
    ...
    //     return Ok(());
    // }
    let Some(logic_entry_block) = split_entry_block_for_flatten(function, entry_block, &mut basic_blocks)? else {
    ...
    let first_block = logic_entry_block;
