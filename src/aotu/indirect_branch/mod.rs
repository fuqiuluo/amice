...
pub struct IndirectBranch {
    enable: bool,
    flags: RedirectBranchFlags,
}

bitflags! {
    struct RedirectBranchFlags: u32 {
...
impl LlvmModulePass for IndirectBranch {
    fn run_pass(
        ...
        if basic_blocks.is_empty() {
            warn!("No basic blocks found in the module, skipping IndirectBranch pass: {}", module.get_name().to_str().unwrap_or("unknown"));
            return PreservedAnalyses::All;
        }
        let basic_block_array = basic_blocks.iter()
        ...
        let global_indirect_branch_table = module.add_global(basic_block_array_ty, None, INDIRECT_BRANCH_TABLE_NAME);
        ...
        for fun in module.get_functions() {
            ...
            for bi in branch_inst_list {
                ...
                let future_branches: Vec<_> = future_branches.iter().filter_map(|&bb| {
                    if let Some(bb) = bb {
                        Some(bb)
                    } else {
                        None
                ...
                let future_branches_address = future_branches.iter().filter_map(|next_basic_block| {
                    unsafe { next_basic_block.get_address() }
                }).map(|addr| {
                    addr.as_value_ref()
                }).collect::<Vec<_>>();

                if future_branches_address.len() < 1 {
                    warn!("(indirect-branch) branch to Meow? future_branches_address.len() < 1!");
                    continue
                }

                let indirect_branch_table = if bi.is_conditional() || !basic_block_array.contains(&future_branches_address[0]) {
                    let basic_block_array_ty = ptr_ty.array_type(future_branches_address.len() as u32);
                    let array_values = future_branches_address.iter()
                ...
                let index = if bi.is_conditional() {
                    ...
                    builder.build_int_z_extend(cond.into_int_value(), i32_ty, "")
                        .map_err(|e| warn!("(indirect-branch) build_int_z_extend failed: {}", e))
                        .ok()
                } else {
                    basic_block_array.iter().position(|&x| x == future_branches_address[0])
                ...
                let Ok(gep) = (unsafe {
                    builder.build_gep(
                        ...
                        .map_err(|e| warn!("(indirect-branch) build gep_index failed: {}", e))
                }) else {
                    continue
                };
                let Ok(loaded_address) = builder.build_load(i8_ptr_ty, gep, "IndirectBranchingTargetAddress")
                    .map_err(|e| warn!("(indirect-branch) build load failed: {}", e)) else {
                    continue
                };

                let Ok(indir_br) = builder.build_indirect_branch(
                    loaded_address.as_basic_value_enum(),
                    &future_branches,
                ).map_err(|e| warn!("(indirect-branch) build indirect branch failed: {}", e)) else {
                    continue
                };
...
impl IndirectBranch {
    pub fn new(enable: bool) -> Self {
        let flags = RedirectBranchFlags::Basic;
