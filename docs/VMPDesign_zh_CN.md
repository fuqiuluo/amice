...
```text
.amice.vm.state.type
.amice.vm.h.<hash>        # 哈希化 private dispatcher / reader helper，不暴露 read_operand/read_varint/dispatch 语义名
...
foo(...) {
  state = init_state(...)
  marshal_args(state, ...)
  call .amice.vm.h.<hash>(&bytecode_foo, &state)
...
`runtime.vm` 也必须使用精确语法：`bank x range x0..x31 type u64`、`bank q range q0..q64 type v128`、`alias name = xN|qN`、`pc: label` 和增强开关 `enhance name = enabled|disabled|func`。alias 缺少 `=`、bank 行存在尾部 token、control-state slot 的类型含多余 token，或 enhancement 名称不在白名单内，都必须在 profile parser/verifier 阶段失败。
